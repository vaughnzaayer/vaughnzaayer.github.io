...
Let's break this down step by step. First, we set a global variable for the thread block dimensions -- in my case, I used `BLOCK_DIM = 32` as it matches the wavefront size on the AMD RX 6800XT GPU. We then calculate the global index of each thread with `row_global` and `col_global`.
...
The AMD RX 6800XT used in the benchmark has 128 MiB in its L3/Infinity Cache. At $N = 2^{11} = 2048$, each matrix takes up 16 MiB for a total of 48 MiB. The entire dataset fits inside of the cache as a result.
